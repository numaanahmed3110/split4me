...
|----------------------|--------------------------------------------------------------------------------------------------|
| 🔒 **Your data, your rules** | Self-hosted by default. Your financial data lives on *your* infrastructure. No tracking. No ads. No "we'll sell your data to advertisers" in the fine print |
...
|---------|--------------|-------------------|
| **Receipt Scanning** | 📸 Photo → structured expense (title, amount, date, category, currency) | Runs on *your* server → calls NVIDIA Nemotron. No S3 upload. No third-party sees your receipts |
...
| **Auto-Categorization** | "Uber to airport" → *Transportation* without lifting a finger | Same Nemotron call, zero config. Learns your patterns |

> **Privacy-first AI**: Your receipt image goes to NVIDIA's API and *nowhere else*. Your voice goes to Deepgram. We don't store them. Your server doesn't log them. Opt-in via env vars — disabled by default.
...
| **Personalized view** | Everyone sees *their* "you owe / you're owed" — not the whole group's laundry |
...
|------|---------------------|
| **Over time** | "Wait, we spent *how much* on coffee last month?" |
...
### Data Portability (Because It's *Your* Data)
...
Copyright (c) 2023–2024 Sebastien Castiel (original author) and the split4me contributors.
...
*split4me — because "you got this one, I'll get the next" only works if someone remembers.*
